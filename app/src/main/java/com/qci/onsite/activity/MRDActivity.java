...
public class MRDActivity extends BaseActivity implements View.OnClickListener {
        ...
        if (resultCode == Activity.RESULT_OK) {
            if (requestCode == 1) {
                if (picUri != null) {
                    ...
                    SaveImage(image2,"fire_extinguisher_present");
    ...
    public void showImageListDialog(ArrayList<String> list, final int position, String from) {
        ...
        if(list.size()==2)
        {
            btn_add_more.setOnClickListener(new View.OnClickListener() {
                @Override
                public void onClick(View v) {
                    Toast toast = Toast.makeText(MRDActivity.this, "You cannot upload more than 2 images.", Toast.LENGTH_SHORT);
    ...
    private void PostLaboratoryData(){
        ...
        for(int i=staffs_personal_files_maintained_list.size(); i<Local_staffs_personal_files_maintained_list.size(); i++)
        {
            ...
            Log.e("UploadImage",Local_staffs_personal_files_maintained_list.get(i) + "fire_extinguisher_present");
            UploadImage(Local_staffs_personal_files_maintained_list.get(i),"fire_extinguisher_present");
            try {
                latch.await();
            }
            catch(Exception ex)
            ...
            if(check==0)
            {
                runOnUiThread(new Runnable() {
                    @Override
                    public void run() {
                        Toast.makeText(MRDActivity.this, "Upload Failed", Toast.LENGTH_SHORT).show();
                    }
                });
                break;
            }
        ...
        if(check==0) {
            runOnUiThread(new Runnable() {
                @Override
                public void run() {
                    Toast.makeText(MRDActivity.this, "Sync Failed", Toast.LENGTH_SHORT).show();
                }
            });
            return;
        }
    ...
    private void SaveImage(final String image_path,final String from){
        if (from.equalsIgnoreCase("fire_extinguisher_present")){
            ...
            Local_image1 = "fire_extinguisher_present";
        }

        Toast.makeText(MRDActivity.this,"Image saved locally",Toast.LENGTH_LONG).show();
